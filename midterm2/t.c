...
********************************************************/
void timer_init();
int body(), tswitch(), do_sleep(), do_wakeup(), do_exit(), do_switch(), do_wait(), do_sleepFor();
int do_kfork();
int scheduler();
...
{
  printf("*******************************************************\n");
  printf(" ps fork switch exit sleep wakeup wait sleepfor timeq \n");
  printf("*******************************************************\n");
}
...
int body() // process body function
{
  ...
  while (1)
  {
    ...
    if (strcmp(cmd, "wakeup") == 0)
      ...
      do_wait();
    else if (strcmp(cmd, "sleepfor") == 0)
      do_sleepFor();
    else if (strcmp(cmd, "timeq")==0)
      printTimeQueue();
  }
}
...
int kfork() // kfork a child process to execute body() function
{
  ...
  // check running process for children:
  // CASE 1 - No children, p becomes first child (child ptr)
  // CASE 2 - Has >0 child, p gets inserted into its sibling list (sibling ptr)
  if (running->child == 0)
  {
    running->child = p;
  }
  ...
  {
    // go to end of existing child's sibling list
...
int do_wait()
{
  ...
  while (answer > 0)
  {
    printf("PROC %d was freed; ", answer);
    printf("EXIT CODE=%d\n", status);
    answer = kwait(status);
  }
}
...
int main()
{
  ...
  VIC_INTENABLE |= (1 << 4); // VIC route timer0,1 at bit4
