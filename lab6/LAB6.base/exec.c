...
int exec(char *cmdline) // cmdline=VA in Uspace
{
  ...
  */
  ...
  // loader return 0 if file non-existant or non-executable
  if (!load(file, p))
  {
    return -1;
  }
  ...
  // fix syscall frame in kstack to return to VA=0 of the new image
  for (i = 2; i < 14; i++)  // clear Umode regs r1-r12
  {
    p->kstack[SSIZE - i] = 0;
  }
  p->kstack[SSIZE-1] = (int)VA(0);  // return uLR = VA(0)
  return (int)p->usp; // will replace r0 in kstack
}
