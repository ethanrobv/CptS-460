...
int kernel_init()
{
  ...
  printf("building Kmode pgdirs at 6MB\n");
  ...
  /*
  // BOOK CODE (?)
  mtable = (int*)0x600000;  // PROC mtables begin at 6MB
  for (i = 0; i < 64; i++)
  {
    for (j = 0; j < 4096; j++)  // zero out high 2048 entries
    {
      mtable[j] = 0;
    }
...
int kgetPA2()
{
  return running->pgdir[2049] & 0xFFFF0000;
}