...
int kfe2(char *commandLine)
{
  ...
  kprintf("KFE: proc %d forked a child %d\n", running->pid, p->pid);
  printQ(readyQueue);
  // **END OF FORK PART**
  ...
  *cp = 0;
  ...
  uLINE = BA + ((int)commandLine - 0x80000000);
  kstrcpy(kline, (char *)uLINE);
  // NOTE: may use uline directly
